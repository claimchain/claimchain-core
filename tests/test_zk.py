...
def test_proof_correct(local_params):
    ...
    assert verify_claim_proof(
            local_params.vrf.pk, '42', claim_proof, 'label', 'content')


def test_proof_incorrect_label(local_params):
    claim_proof = compute_claim_proof('42', 'label', 'content')
    assert not verify_claim_proof(
            local_params.vrf.pk, '1', claim_proof, 'label', 'content')


def test_proof_incorrect_label(local_params):
    claim_proof = compute_claim_proof('42', 'label', 'content')
    assert not verify_claim_proof(
            local_params.vrf.pk, '42', claim_proof, 'incorrect label', 'content')


def test_proof_incorrect_message(local_params):
    claim_proof = compute_claim_proof('42', 'label', 'content')
    assert not verify_claim_proof(
            local_params.vrf.pk, '42', claim_proof, 'label', 'something else')
...
def test_proof_incorrect_pubkey(local_params):
    ...
    assert not verify_claim_proof(
            other_params.vrf.pk, '42', claim_proof, 'label', 'content')


def test_vrf_deterministic_value(local_params):
    claim_proof1 = compute_claim_proof('42', 'label', 'content')
    claim_proof2 = compute_claim_proof('42', 'label', 'content')
